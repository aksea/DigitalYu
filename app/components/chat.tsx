  ...
  return (
    <div
      className={clsx(styles["chat-input-action"], "clickable", {
        [styles["chat-input-action-selected"]]: props.selected,
      })}
      onClick={() => {
        props.onClick();
...
function _Chat() {
  ...
  const toggleFeedback = (message: ChatMessage, value: "up" | "down") => {
    chatStore.updateTargetSession(session, (session) => {
      const target = session.messages.find((m) => m.id === message.id);
      if (!target) return;
      target.feedback = target.feedback === value ? undefined : value;
    });
  };

  const editComment = async (message: ChatMessage) => {
    const newComment = await showPrompt(
      "实验评论",
      message.comment ?? "",
      6,
    );
    chatStore.updateTargetSession(session, (session) => {
      const target = session.messages.find((m) => m.id === message.id);
      if (!target) return;
      target.comment = newComment.trim() ? newComment : undefined;
    });
  };
                      ...
                      >
                        <div className={styles["chat-message-container"]}>
                          ...
                          <div className={styles["chat-message-meta"]}>
                            {message.role === "assistant" && !isContext && (
                              <div className={styles["chat-message-feedback"]}>
                                <button
                                  type="button"
                                  aria-label="comment"
                                  className={clsx(
                                    styles["chat-message-feedback-btn"],
                                    {
                                      [styles["chat-message-feedback-active"]]:
                                        !!message.comment,
                                    },
                                  )}
                                  onClick={() => editComment(message)}
                                >
                                  <CommentIcon />
                                </button>
                                <button
                                  ...
                                  onClick={() => toggleFeedback(message, "up")}
                                >
                                  <ThumbUpIcon />
                                </button>
                                <button
                                  ...
                                  onClick={() => toggleFeedback(message, "down")}
                                >
                                  <ThumbDownIcon />
                                </button>
                              </div>
                ...
                <textarea
                  ...
                  placeholder={
                    hasParticipantName
                      ...
                      : Locale.Chat.InputNamePlaceholder
                  }
